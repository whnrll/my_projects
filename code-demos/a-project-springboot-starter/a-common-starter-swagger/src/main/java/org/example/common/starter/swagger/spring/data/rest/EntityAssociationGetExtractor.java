...
public class EntityAssociationGetExtractor implements EntityAssociationOperationsExtractor {
  ...
  public List<RequestHandler> extract(EntityAssociationContext context) {
    ...
    springfox.documentation.spring.data.rest.ActionSpecification get = new springfox.documentation.spring.data.rest.ActionSpecification(
        String.format("%s%s",
    ...
    handlers.add(new springfox.documentation.spring.data.rest.SpringDataRestRequestHandler(context.getEntityContext(), get));
    return handlers;
  }
}
