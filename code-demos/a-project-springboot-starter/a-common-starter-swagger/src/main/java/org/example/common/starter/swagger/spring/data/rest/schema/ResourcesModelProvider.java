 ...
 *
 */
package org.example.common.starter.swagger.spring.data.rest.schema;
...
class ResourcesModelProvider implements SyntheticModelProviderPlugin {
  ...
  public List<ModelProperty> properties(ModelContext context) {
    ...
    List<ResolvedType> typeParameters = resourceType.getTypeParameters();
    Class<?> type = typeParameters.get(0).getErasedType();
    ResolvedType embedded = resolver.resolve(springfox.documentation.spring.data.rest.schema.EmbeddedCollection.class, type);
  ...
  public Set<ResolvedType> dependencies(ModelContext context) {
    ...
    return newHashSet(
        resolver.resolve(springfox.documentation.spring.data.rest.schema.EmbeddedCollection.class, type),
