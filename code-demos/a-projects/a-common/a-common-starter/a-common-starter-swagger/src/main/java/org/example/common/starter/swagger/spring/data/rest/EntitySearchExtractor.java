...
class EntitySearchExtractor implements EntityOperationsExtractor {
  ...
  public List<RequestHandler> extract(EntityContext context) {
    ...
    for (MethodResourceMapping mapping : searchMappings.getExportedMappings()) {
      HandlerMethod handler = new HandlerMethod(
          ...
          mapping.getMethod());
      springfox.documentation.spring.data.rest.ActionSpecification spec = new springfox.documentation.spring.data.rest.ActionSpecification(
      ...
      handlers.add(new springfox.documentation.spring.data.rest.SpringDataRestRequestHandler(context, spec));
    }
    return handlers;
  }
  ...
  private ResolvedMethodParameter transferResolvedMethodParameter(ResolvedMethodParameter src) {
    ...
    if (param.isPresent()) {
      return src.annotate(springfox.documentation.spring.data.rest.SynthesizedAnnotations.requestParam(param.get().value()));
    }
    return src;
  }
}
