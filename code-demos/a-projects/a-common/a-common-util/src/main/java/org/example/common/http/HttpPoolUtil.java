...
public class HttpPoolUtil {
    ...
    private HttpPoolUtil() {
        ...
        RequestConfig defaultRequestConfig = RequestConfig.custom().setConnectTimeout(connectionTimeout) // 连接超时时间
        ...
        // 创建 http client 连接
        client = HttpClients.custom().setConnectionManager(connectionManager) // 连接管理器
    ...
    /**
     * HTTP POST 调用, 附带agent信息 方法用途: <br>
     * 实现步骤: <br>
     * 
     * @return
     * @throws Exception
